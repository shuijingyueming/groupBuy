...
function PageNavCreate(id, props) {
    ...
    $('<div class="page-nav-inner clearfloat">' +
        ...
        '<input type="text" id="shuzi" values="" autocomplete="off" onfocus="this.type=password"/>' +
    ...
    if (this.pageCount <= this.perPageNum || this.pageCount > this.perPageNum) {
        //总页数写入placeholder
        this.pageNavInput.children('input').val("").attr({"placeholder": this.pageCount, "max": this.pageCount});
    } else {
        //总页数写入placeholder
        this.pageNavInput.children('input').val("").attr({"placeholder": 0, "max": 0});
    }
    ...
    if (this.pageCount <= this.perPageNum) {
        ...
        $('<li class="page-nav-next">' +
            ...
            '</a>' +
            '</li>').appendTo(this.pageNavUl);
    } else if (this.pageCount > this.perPageNum) {//总页数大于每页按钮数
        //重写一遍翻页按钮 START
        this.pageNavUl.html("");
        $('<li class="page-nav-first">' +
            '<a href="javascript:void(null)" aria-label="First page" pagenum="1" >' +
        ...
        this.pageNavUl.html("");
        $('<li class="page-nav-first">' +
            '<a href="javascript:void(null)" aria-label="First page" pagenum="0" >' +
            '<span aria-hidden="true">&laquo;</span>' +
            '</a>' +
            '</li>' +
            '<li class="page-nav-prev">' +
            '<a href="javascript:void(null)" aria-label="Previous" pagenum="0" >' +
            '<span aria-hidden="true">&lt;</span>' +
            '</a>' +
            '</li>').appendTo(this.pageNavUl);
        $('<li class="page-nav-next">' +
            '<a href="javascript:void(null)" aria-label="Last page"  pagenum="0" >' +
            '<span aria-hidden="true">&gt;</span>' +
            '</a>' +
            '</li>' +
            '<li class="page-nav-last">' +
...
PageNavCreate.prototype.afterClick = function (func) {
    ...
    this.pageNavInput.children('button').off("click").on("click", function (event) {
        var inputVal = parseInt($('#shuzi').val());
